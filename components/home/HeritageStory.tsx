...
export function HeritageStory({ eyebrow, title, lede, support }: HeritageStoryProps) {
  return (
    <section className="relative overflow-hidden bg-charcoal px-5 py-28 text-ivory md:px-8 md:py-36 lg:px-12">
      <LotusField />

      <div className="relative mx-auto max-w-[1440px]">
        <Reveal>
          <p className="text-[11px] uppercase tracking-[0.38em] text-gold">{eyebrow}</p>

          <h2 className="mt-6 max-w-[16ch] font-display text-4xl leading-[1.12] sm:text-6xl lg:text-[4.4rem]">
            {title}
          </h2>

          <span className="mt-8 block h-px w-16 bg-gold" aria-hidden />

          <p className="mt-8 max-w-2xl text-[17px] leading-relaxed text-ivory/75">
            {lede}
          </p>
          <p className="mt-6 max-w-xl text-sm leading-relaxed text-ivory/50">{support}</p>
        </Reveal>
      </div>
    </section>
  );
...
function LotusField() {
  ...
  return (
    <svg
      aria-hidden
      viewBox="0 0 800 400"
      className="pointer-events-none absolute -right-16 top-1/2 w-[720px] -translate-y-1/2 text-gold opacity-[0.07]"
    >
